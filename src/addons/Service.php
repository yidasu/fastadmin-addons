...
class Service
{
    ...
    public static function refresh()
    {
        ...
        $addonsFile = Env::get('root_path') . str_replace("/", DIRECTORY_SEPARATOR, "public/assets/js/addons.js");
...
EOD;
            ...
            if ($conflictFiles) {
                ...
                try {
                    foreach ($conflictFiles as $k => $v) {
                        $zip->addFile(Env::get('root_path') . $v, $v);
                    }
                    $addonsBackupDir = self::getAddonsBackupDir();
        ...
        if (config('fastadmin.addon_pure_mode') || !$list) {
            if ($config && isset($config['files']) && is_array($config['files'])) {
                foreach ($config['files'] as $index => $item) {
                    ...
                    if (is_file(Env::get('root_path') . $item)) {
                        @copy(Env::get('root_path') . $item, $addonDir . $item);
                    }
                }
                $list = $config['files'];
            }
    ...
    {
        return Env::get('config_path') . DIRECTORY_SEPARATOR . 'addons.php';
    }
    ...
    {
        $dir = Env::get('runtime_path') . 'addons' . DIRECTORY_SEPARATOR;
        if (!is_dir($dir)) {
            @mkdir($dir, 0755, true);
        }
        return $dir;
    ...
    {
        $assetsDir = Env::get('root_path') . str_replace("/", DIRECTORY_SEPARATOR, "public/assets/addons/{$name}/");
        return $assetsDir;
    }
